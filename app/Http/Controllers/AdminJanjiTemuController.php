<?php namespace App\Http\Controllers;
	...
	class AdminJanjiTemuController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Pasien Id","name"=>"pasien_id","join"=>"pasiens,id"];
			$this->col[] = ["label"=>"Nominal","name"=>"nominal"];
			$this->col[] = ["label"=>"Bank","name"=>"bank"];
			$this->col[] = ["label"=>"Tgl Konsultasi Mulai","name"=>"tgl_konsultasi_mulai"];
			$this->col[] = ["label"=>"Tgl Konsultas Selesai","name"=>"tgl_konsultas_selesai"];
			...
			$this->form[] = ['label'=>'Janji Temu Konselor Id','name'=>'janji_temu_konselor_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'janji_temu_konselor,id'];
			$this->form[] = ['label'=>'Pasien Id','name'=>'pasien_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'pasien,id'];
			$this->form[] = ['label'=>'Nominal','name'=>'nominal','type'=>'number','validation'=>'required|integer|min:0','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Bank','name'=>'bank','type'=>'number','validation'=>'required|integer|min:0','width'=>'col-sm-10'];
			...
			$this->form[] = ['label'=>'Konselor Users Id','name'=>'konselor_users_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'konselor_users,id'];
			$this->form[] = ['label'=>'Catatan Kasus','name'=>'catatan_kasus','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			$this->form[] = ['label'=>'Konselor Id','name'=>'konselor_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'konselor,id'];
			...
			//$this->form[] = ["label"=>"Bank","name"=>"bank","type"=>"number","required"=>TRUE,"validation"=>"required|integer|min:0"];
			...
			//$this->form[] = ["label"=>"Tgl Konsultas Selesai","name"=>"tgl_konsultas_selesai","type"=>"datetime","required"=>TRUE,"validation"=>"required|date_format:Y-m-d H:i:s"];
			//$this->form[] = ["label"=>"Durasi Konsultasi","name"=>"durasi_konsultasi","type"=>"number","required"=>TRUE,"validation"=>"required|integer|min:0"];
			//$this->form[] = ["label"=>"Konselor Users Id","name"=>"konselor_users_id","type"=>"select2","required"=>TRUE,"validation"=>"required|integer|min:0","datatable"=>"konselor_users,id"];
